...
- **الصفحات اللي فاضلة موقع (بعد 3.2 — ٢٨ من ٦٧):** `orders/new` (طلب يدوي — فورم ٩٠٠ سطر)، `analytics/{live,reports,signal}`،
  `marketing/{campaigns,channels}`، `inventory/branches`، `products/import`، `settings/{checkout,orders,receipt,seo,pages,domain,email,whatsapp}`،
...
3. الجاي في التطبيق: باقي صفحات اللوحة اللي لسه موقع — بالترتيب المقترح (الدفع والشحن اتعملوا في 2.9، والبوستات في 3.0، والنشر التلقائي والحسابات في 3.1، والفريق والأجهزة وسجل النشاط في 3.2): طلب جديد يدوي `/dashboard/orders/new`، إعدادات الطلبات والشيك أوت وواتساب والبريد، تقارير التحليلات والزوار دلوقتي، الحملات وقنوات البيع، الفروع واستيراد المنتجات، إضافة مستقبِل إشعارات من التطبيق، ثم الاستوديو
