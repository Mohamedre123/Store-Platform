...
الشاشات الأربعة دي بياناتها في `shell/store-settings-api.ts` وستايلها في `shell/styles-store-settings.ts` (بادئة `st-`)، وقطعها المشتركة في `shell/settings-forms.tsx`:
...
- **الصفحات اللي فاضلة موقع (بعد 3.4 — ٢٣ من ٦٧):** `analytics/{live,reports,signal}`،
  `marketing/{campaigns,channels}`، `inventory/branches`، `products/import`، `settings/{receipt,seo,pages,domain}`،
  `account`، `storefront` و`storefront/customize`، `studio`، `plugins`، `landing` و`landing/[id]`، `developers`، `experiments`، `marketplace`، `markets`، `admin`.
...
3. الجاي في التطبيق: باقي صفحات اللوحة اللي لسه موقع — بالترتيب المقترح (الدفع والشحن اتعملوا في 2.9، والبوستات في 3.0، والنشر التلقائي والحسابات في 3.1، والفريق والأجهزة وسجل النشاط في 3.2، والطلب اليدوي في 3.3، وإعدادات الطلبات والشيك أوت وواتساب والبريد في 3.4): صفحة الطلب والإيصال والسيو وصفحات المتجر والنطاق، تقارير التحليلات والزوار دلوقتي، الحملات وقنوات البيع، الفروع واستيراد المنتجات، إضافة مستقبِل إشعارات من التطبيق، ثم الاستوديو
