...
| `src/lib/team-invites.ts` + `src/lib/after-verify.ts` + `settings/team/actions.ts` | `src/app/(auth)/join/*` و`/verify` و`/login` وصفحة الفريق | — (الموقع بس؛ التطبيق بيعرضها كصفحة منصة — قسم 7ز) |
...
3. الجاي في التطبيق: باقي صفحات اللوحة اللي لسه موقع — بالترتيب المقترح (الدفع والشحن اتعملوا في 2.9، والبوستات في 3.0، والنشر التلقائي والحسابات في 3.1): إضافة مستقبِل إشعارات من التطبيق، ثم الاستوديو
