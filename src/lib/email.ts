...
/**
 * الافتراضي `platform` — رجوع مقصود للوضع اللي كان شغّالًا.
 *
 * ## اللي حصل
 * اسم المتجر اتحطّ على الرسايل يوم ٢٤ أغسطس، والسبام بدأ بعده.
 * فجرّبنا التلات هويات في نفس اللحظة لنفس العنوان بنفس المحتوى —
 * **وتلاتتهم راحوا السبام**، حتى هوية المنصة اللي كانت شغّالة.
 *
 * يعني الترويسة مش السبب الوحيد: سمعة النطاق نفسها نزلت (رسايل
 * اختبار كتير في يومين لعناوين ما تفاعلتش معاها). والسمعة بتترجع
 * بالتفاعل والوقت لا بالكود.
 *
 * فبنرجع للهوية اللي ليها أطول تاريخ إرسال ناجح: هوية واحدة بتراكم
 * سمعة أسرع من هوية لكل متجر — كل عنوان جديد بيبدأ من الصفر.
 *
 * **واسم المتجر ما بيضيعش:** موجود في موضوع الرسالة، وفي الشعار
 * فوقها، وفي نصّها. اللي بيتغيّر ترويسة `From` بس.
 *
 * لما السمعة ترجع، غيّر `EMAIL_SENDER_STYLE` لـ`store-name` وجرّب
 * تاني من صفحة بريد المتجر.
 */
function senderStyle(): SenderStyle {
  const v = process.env.EMAIL_SENDER_STYLE
  return v === 'platform' || v === 'store-name' || v === 'store-address' ? v : 'platform'
}
