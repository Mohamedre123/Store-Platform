...
export const PLUGINS: PluginDef[] = [
...

  {
    ...
    desc:
      ...
      'ولو وقف أو خلصت حصّته، بيحوّل العميل على واتسابك بدل ما يسيبه.',
    group: 'ai',
    custom: 'gemini',
    secretFields: ['apiKey'],
    fields: [{ key: 'apiKey', label: 'مفتاح Gemini API', placeholder: 'مفتاحك من Google AI Studio' }],
    where:
      'من aistudio.google.com ← Get API key. أي مفتاح بيطلع من هناك بيشتغل — إحنا بنتحقق منه بنداء حقيقي مش بشكله. ' +
      'المفتاح المجاني بيشتغل بحد يومي بسيط؛ **فعّل الفوترة** عشان البوت ما يقفش قدام عميل بيسأل.',
  },

  {
    slug: 'gemini_pro',
    name: 'مساعدك في إدارة المتجر',
    desc:
    ...
    secretFields: ['apiKey'],
    fields: [
      { key: 'apiKey', label: 'مفتاح Gemini API (اختياري)', placeholder: 'سيبه فاضي عشان يستخدم مفتاح البوت' },
    ],
    where:
      'من aistudio.google.com ← Get API key، وأي صيغة مفتاح بتشتغل. ' +
      'سيبه فاضي عشان يستخدم مفتاح «الردّ على عملائك». ' +
      '**المساعد ده محتاج مفتاح عليه فوترة**: هو بيقرا بيانات متجرك في كل سؤال وبيعدّل الصور، ' +
      'فاستهلاكه أعلى بكتير من البوت والمفتاح المجاني بيقف معاه من أول شوية.',
  },
...

  {
    ...
    fields: [],
    where:
      'بيشتغل بمفتاح Gemini اللي حاططه في «الردّ على عملائك» أو «مساعدك في إدارة المتجر» — ' +
      'مش محتاج مفتاح تالت. **بس لازم يكون عليه فوترة**: توليد الصور أغلى بكتير من النص ' +
      'والحصّة المجانية بتقف من أول شوية. ' +
