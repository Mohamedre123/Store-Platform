 ...
 */
function describe(ua: string | null): { device: SessionView['device']; label: string } {
  const s = ua ?? ''

  const device: SessionView['device'] = /iPad|Tablet/i.test(s) ? 'tablet' : /Mobi|Android|iPhone/i.test(s) ? 'mobile' : 'desktop'
...
/**
 * الجلسات السارية للمستخدم — صفحة «الأجهزة والجلسات» ومسار التطبيق (`/api/app/sessions`).
 *
 * السارية بس: الجلسة المنتهية مش «جهاز داخل» — عرضها بيخلّي التاجر يشوف عشرين
 * صفًا ويفتكر إن عشرين حد فاتحين حسابه.
 */
export async function loadSessions(userId: string): Promise<SessionView[]> {
