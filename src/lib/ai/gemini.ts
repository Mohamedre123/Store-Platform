...
function isDeadModel(e: GeminiError): boolean {
  return (
    ...
    /*
      جوجل قالت بنفسها إن الموديل ده مش متاح (503).

      **ده كان أهم سطر ناقص.** الـ503 كان بيتلمّ مع باقي أخطاء الخادم
      في رسالة «خدمة جوجل مش مستجيبة»، فما كانش بيعدّي الفحص ده —
      والرجوع للبديل ما كانش بيشتغل أبدًا عليه.

      والفرق حقيقي: على مفتاح واحد وفي نفس الثانية، موديل ردّ في ٩٤٥
      مللي وموديل تاني ردّ 503. يعني الحل موديل تاني، مش الاستنّى —
      والرسالة كانت بتقول للتاجر يستنّى على حاجة الاستنّى مش هيصلّحها.
    */
...
function classify(status: number, body: string): GeminiError {
  ...
  /*
    503 معناه **الموديل ده** مش متاح، مش إن جوجل واقعة.

    وده فرق حاسم: جرّبنا على مفتاح واحد في نفس الثانية —     ردّ في ٩٤٥ مللي و ردّ 503. فالحل موديل تاني، مش
    الاستنّى.

    وكان بيتلمّ مع باقي الـ5xx في رسالة «خدمة جوجل مش مستجيبة»، وبكده
    ما بيعدّيش فحص الموديل الميت تحت — فالبديل ما بيشتغلش أبدًا،
    والتاجر بيشوف رسالة بتقوله يستنّى على حاجة الاستنّى مش هيصلّحها.
  */
  if (status === 503) {
    return { kind: 'network', message: 'الموديل ده مش متاح دلوقتي عند جوجل.' }
  }
  if (status >= 500) {
