...
export type ListingSettings = {
  ...
  /**
   * زر البطاقة في بلوكات الصفحة الرئيسية.
   *
   * كان مفتاحًا بقيمتين (`showQuickAdd`): زر إضافة أو ولا حاجة. وقيمة
   * تالتة موجودة في الكود من الأول — «الخيارات على البطاقة» — ومكانش
   * ليها أي مكان يتظبّط منه غير محرّر البلوك الواحد.
   *
   * فالتاجر اللي قفل الزر ورجّعه بياخد «إضافة» بدل «الخيارات»، وشايف
   * إن الخيارات اختفت من غير سبب.
   *
   * والمفتاح القديم بيفضل الاحتياطي عشان المتاجر المحفوظة ما تتغيّرش.
   */
  cardAction?: 'none' | 'add' | 'choose'
  perPage: number
}
